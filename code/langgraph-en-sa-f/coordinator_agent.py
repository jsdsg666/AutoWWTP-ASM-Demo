...
from . import config
from .agents import single_agent, single_self_check_agent
...
def report_summary(state: AgentState) -> dict:
    ...
    log = list(state.get("log", []))
    if state.get("last_returncode") not in (0, None):
        final_status = "failed"
        log.append(f"[summary] run_model failed with returncode={state.get('last_returncode')}.")
    elif state.get("config_ok") is False:
        final_status = "failed"
        log.append("[summary] Config validation failed.")
    elif state.get("fatal_error"):
        final_status = "failed"
    elif state.get("last_returncode") == 0:
        final_status = "ok"
    else:
...
def route_after_single(state: AgentState):
    """End directly if single-agent generation or self-check fails."""
    if state.get("fatal_error") or state.get("status") == "failed":
        return "report_summary"
    return "next"
...
def build_graph():
    ...
    g.add_node("single_self_check", single_self_check_agent)
    ...
    g.add_conditional_edges(
        "prepare_environment",
        route_after_prepare,
        {"single_agent": "single_agent", "report_summary": "report_summary"},
    )
    g.add_conditional_edges(
        "single_agent",
        route_after_single,
        {"next": "single_self_check", "report_summary": "report_summary"},
    )
    g.add_conditional_edges(
        "single_self_check",
        route_after_single,
        {"next": "run_model", "report_summary": "report_summary"},
    )
