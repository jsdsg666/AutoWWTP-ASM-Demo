...
def plan_agent(state: AgentState) -> dict:
    ...
    knowledge = state.get("process_context_md") or (
        config.KNOWLEDGE_PATH.read_text(encoding="utf-8") if config.KNOWLEDGE_PATH.exists() else ""
    )
...
# Process Context (midoutput/WWTPProcessContext.md)
{knowledge}
