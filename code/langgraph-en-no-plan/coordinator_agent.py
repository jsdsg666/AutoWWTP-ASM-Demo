...
def route_after_modeling(state: AgentState):
    """End directly if direct config generation fails."""
    if state.get("fatal_error") or state.get("status") == "failed":
        return "report_summary"
...
def build_graph():
    ...
    g.add_edge("knowledge_agent", "modeling_agent")
    g.add_conditional_edges(
        "modeling_agent",
        route_after_modeling,
        {"reflection_agent": "reflection_agent", "report_summary": "report_summary"},
    )
    g.add_conditional_edges(
        "reflection_agent",
